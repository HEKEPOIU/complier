...
int f(int i, int c) {
  ...
  int s = 0, j = 0;
  for (j = 0; j < N; j++) { // iter the every row number

    int col = 1 << j; // which columns select
    if ((c & col) == 0) // can't find where to get num, because all available columns be select.
      continue;

    int x = m[i][j] + f(i + 1, c - col); //cal the sum
    if (x > s) 
      s = x;
  }
  memo[key] = s;
