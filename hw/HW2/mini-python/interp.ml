...
let rec expr ctx = function
  ...
  | Ebinop
      ...
      | Bdiv, Vint n1, Vint n2 ->
          if n2 <> 0 then Vint (n1 / n2) else error "divide by 0"
      | Bmod, Vint n1, Vint n2 ->
          if n2 <> 0 then Vint (modulo n1 n2) else error "divide by 0"
  ...
  | Ecall ({ id = f }, el) -> (
      ...
      (* Create a new stack for functions call *)
      List.iter2 (fun { id = x } e -> Hashtbl.add ctx' x (expr ctx e)) args el;
      (*allocate the call args*)
      try
        stmt ctx' body;
        Vnone
      with Return v -> v
      (*try to call function as bottom*))
...
and stmt ctx = function
  ...
  | Sassign ({ id }, e1) -> Hashtbl.replace ctx id (expr ctx e1)
